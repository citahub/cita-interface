...
function gen_modrs_for_protos () {
    ...
    find ../protos -maxdepth 1 -name "*.proto" \
            -exec basename {} \; \
            | sort \
...
function remove_all_generated_code () {
    ...
    sed -i "/^${replace} begin:$/,/^${replace} end.$/{//!d}" router.rs
    sed -i "/^${replace} begin:$/,/^${replace} end.$/{//!d}" lib.rs
    sed -i "/^${replace} begin:$/,/^${replace} end.$/{//!d}" autoimpl.rs
}
...
function generate_impls_for_msg () {
    ...
    local newcode="$4"
    sed -n '/^    oneof content {$/,/^    }$/p' "../protos/communication.proto" \
